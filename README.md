...
输入学号 `test` 可直接登录一次性测试账号。服务端会为每次登录生成独立身份，多个听课老师可以同时使用而不会互相顶掉。`students.csv` 中以大写 `A` 开头的学号是固定账号；新建场次时，这些账号的工作数据和登录会延续到新场次，普通学生只延续名单并重置场次数据，测试账号不会延续。
...
- 每次公开课前通过教师端新建场次。新场次会复制当前 CSV 名单；普通账号不会复制设计、消息、Memory 开关/条目和用量，`A` 开头的固定账号则完整延续这些数据和登录。

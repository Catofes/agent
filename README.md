...
一个面向公开课的极简 Agent 平台：Go 单二进制、SQLite 单文件、学生/教师/大屏三个内嵌页面。平台提供学号登录、Agent 设计、可确认的长期 Memory、计算器工具调用、行动记录、教师状态墙、全班锁定、场次隔离和投屏。
...
Memory MVP 默认关闭，候选需由学生确认，只在同一课堂场次内跨对话生效；具体语义、限额和验收见 [Memory MVP 设计](doc/MemoryMVP设计.md)。zip 导出、随机点名、优秀池、联网搜索、绘图和多智能体演示仍在 [todo.md](todo.md) 的后续版本清单中。

...
func Load(version string) (Config, error) {
	...
	flag.IntVar(&c.MaxMemoryItems, "max-memory-items", envInt("MAX_MEMORY_ITEMS", 100), "maximum memory candidates and confirmed items per student and run")
	flag.IntVar(&c.MaxMemoryChars, "max-memory-chars", envInt("MAX_MEMORY_CHARS", 400), "character limit per memory item")
	flag.IntVar(&c.MaxMemoryTokens, "max-memory-tokens", envInt("MAX_MEMORY_TOKENS", 1200), "estimated token limit for memory injected into one model turn")
